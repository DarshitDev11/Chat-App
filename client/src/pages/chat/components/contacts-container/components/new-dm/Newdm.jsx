...
import { SEARCH_CONTACTS_ROTES } from '@/utils/constants'
...
const Newdm = () => {
  ...
  const searchContacts = async (searchTerm)=>{
    try {
      if(searchTerm.length > 0){
        const responce = await apiClient.post(SEARCH_CONTACTS_ROTES,
          {searchTerm},
  ...
  return (
    ...
    <Dialog open={openNewcontactModal} onOpenChange={setopenNewcontactModal}>
              ...
              <Avatar className='h-12 w-12 rounded-full overflow-hidden'>
                {
                  ...
                  />):(
                    <div className={`upperacse h-12 w-12 text-lg border-[1px] flex items-center justify-center rounded-full ${getColor(contact.color)}`}>
      ...
      {
      searchedContacts.length <= 0 &&  <div className="flex-1 md:flex flex-col items-center justify-center mt-5 md:mt-0 duration-100 transition-all">
      <Lottie
        loop={true}
        autoplay={true}
